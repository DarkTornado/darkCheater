...
/*
ModPE.overrideTexture("images/items-opaque/darkness_0.png", "https:\/\/www.dropbox.com\/s\/bi8ky393wk4kive\/darkness_0.png?dl=1");
ModPE.overrideTexture("images/darkness_0.png", "https:\/\/www.dropbox.com\/s\/bi8ky393wk4kive\/darkness_0.png?dl=1");
new java.lang.Thread({
run : function(){
try{
java.lang.Thread.sleep(5000);
ModPE.setItem(1000, "darkness", 0, "실험");
}catch(e){print(e);}
}
}).start();
*/
...
i6.setText("NMC 서비스(Nickname Matching Service, 닉네임 일치 확인 서비스)는 로그인 시, 다크 치터 닉네임과 마인크래프트 pe 닉네임이 일치하는지를 확인하여, 일치하면 로그인을 하고, 일치하지 않으면 로그인을 하지 않는 서비스입니다. 항목 입력이 없으면 신청 안함으로 구분합니다.");
i6.setTextSize(13);
...
Dark.toast("아이디 중복 확인을 하지 않으셨거나, 이미 사용중인 아이디를 입력하셨습니다.");
}
}
else{
...
if(invenSave&&Entity.getHealth(ad)>0){
if(isDied){
for(var n=0;n<36;n++){
...
layout.addView(btnT[n]);
var button = new Array();
var menu = ["웹 브라우저 설정", "UI 변경", "버튼 위치 수정", "메뉴 위치 수정", "위젯 위치 수정", "추가 파일 다운로드", "지도 범위 설정", "엔티티 스폰 방지 설정", "메뉴 색상 설정", "이용 약관 보기", "다크 치터 테마 관리", "회원 가입"];
for(var n in menu)
...
loadWeb.setWebChromeClient(new android.webkit.WebChromeClient());
loadWeb.setWebViewClient(new android.webkit.WebViewClient());
loadWeb.loadUrl("https:\/\/raw.githubusercontent.com\/DarkTornado\/darkCheater\/master\/potionRecipe.jpeg");
layout.addView(loadWeb);
var scroll = android.widget.ScrollView(ctx);
scroll.addView(layout);
dialog.setView(scroll);
...
var layout = new android.widget.LinearLayout(ctx);
layout.setOrientation(1);
var menusT = ["PVP 방지", "서버원 공중부양 방지", "표지판 TP", "명령어 단축키", "채팅 단축키", "연속 채팅 방지", "블록 설치 내역 기록", "서버원 접속 방지", "하이퍼 로그 사용", "블록 설치 방지", "The Seed", "서버장 사칭 방지", "서버 목록 숨기기"];
var btnT = new Array();
for(var n in menusT){
btnT[n] = new android.widget.ToggleButton(ctx);
btnT[n].setTextOn(menusT[n]);
...
var btns = new Array();
var menus = ["블록 설치 내역", "대화 내용 보기", "채팅창 비우기", "채팅 조작", "채팅 무시 설정", "서버원 접속 내역", "하이퍼 로그 보기", "플레이어 이동 금지", "내 ip 확인"];
for(var n in menus)
btns[n] = makeButton(menus[n]);
btns[0].setOnClickListener(new android.view.View.OnClickListener(){
onClick: function(v){
